            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">Rp {{number_format($withdrawal->amount_requested)}}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Date</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{$withdrawal->created_at->format('M d, Y')}}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Fundraiser</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{$withdrawal->fundraising->fundraiser->user->name}}</h3>
                    </div>
                    @if($withdrawal->has_received)
                    <span class="w-fit text-sm font-bold py-2 px-3 rounded-full bg-green-500 text-white">
                        SUCCESS
                    </span>
                    @elseif($withdrawal->has_sent)
